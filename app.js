    ...
    {$page: page}, function(error, rows) {
      if (error) {
        ...
        for (var i = 0; i < rows.length; i++) {
          ...
          result += '<div id="' + rows[i].ArtistId + '"><span class="numbers">' + ((page - 1) * artistsPerPage + i + 1) + '.</span>' +
...
app.get('/pages', function(request, response) {
  db.get("SELECT COUNT(*) AS Artists FROM Artist", function(error, row) {
    if (error) {
      ...
      response.send({pages: Math.ceil(row.Artists / artistsPerPage)});
  });
});
